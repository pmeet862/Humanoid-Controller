...
public class HumanoidLandController : MonoBehaviour
{
    ...
    [Header("Movement")]
    [SerializeField] float _movementMultiplier = 75.0f;
    [SerializeField] float _rotationSpeedMultiplier = 120.0f;
    [SerializeField] float _pitchSpeedMultiplier = 120.0f;
    [SerializeField] float _walkSpeedMultiplier = 30.0f;
    ...
    [SerializeField] float _firstStepVelocityMultiplier = 0.1f;
    [SerializeField] bool _playerIsAscendingStairs = false;
    [SerializeField] bool _playerIsDescendingStairs = false;
    [SerializeField] float _ascendingStairsMovementMultiplier = 1.0f;
    [SerializeField] float _descendingStairsMovementMultiplier = 1.2f;
    [SerializeField] float _maximumAngleOfApproachToAscend = 75.0f;
    float _playerHalfHeightToGround = 0.0f;
    float _maxAscendRayDistance = 0.0f; //This gets calculated in Awake() and is based off _maxStepHeight and max approach angle of _maximumAngleOfApproachToAscend
    ...
    int _numberOfStepsDetectsRays = 0;
    ...
    [SerializeField][Range(0.0f, 1.8f)] float _headCheckRadiousMultiplier = 0.9f;
    ...
    private void Awake()
    {
        ...
        _numberOfStepsDetectsRays = Mathf.RoundToInt(((_maxStepHeight * 100.0f) * 0.5f) + 1.0f);
        _rayIncrementAmount = _maxStepHeight / _numberOfStepsDetectsRays;
    ...
    private void FixedUpdate()
    {
        ...
        if (_input.MoveIsPressed && _input.WalkIsPressed && !_playerIsCrouching && (_playerIsGrounded || _playerIsAscendingStairs || _playerIsDescendingStairs))
        {
            _playerMoveInput = PlayerWalk();
        }
        else
        {
        ...
        _playerMoveInput *= _rigidbody.mass; //Note: For dev purpose.
    ...
    {
        // Vector3 calculatedPlayerWalkSpeed = _playerMoveInput;
        // if (_input.MoveIsPressed && _input.WalkIsPressed)
        // {
        //     calculatedPlayerWalkSpeed = calculatedPlayerWalkSpeed * _walkSpeedMultiplier;
        // }
        // return calculatedPlayerWalkSpeed;
        return new Vector3(_playerMoveInput.x * _walkSpeedMultiplier, _playerMoveInput.y, _playerMoveInput.z * _walkSpeedMultiplier);
    }
    ...
    private bool PlayerGroundCheck()
    {
        ...
        return ((_playerCenterToGroundDistance >= _capsuleCollider.bounds.extents.y - _groundCheckDistanceTolerance) && (_playerCenterToGroundDistance <= _capsuleCollider.bounds.extents.y + _groundCheckDistanceTolerance));
    }
    ...
    private Vector3 AscendStairs(Vector3 calculatedStepInput)
    {
        ...
        {
            float calculatedVelDistance = _isFirstStep ? (_rigidbody.velocity.magnitude * _firstStepVelocityMultiplier) + _capsuleCollider.radius : _capsuleCollider.radius;
            float ray = 0.0f;
            List<RaycastHit> raysThatHit = new List<RaycastHit>();
            for (int x = 1; x <= _numberOfStepsDetectsRays; x++, ray += _rayIncrementAmount)
            {
            ...
            if (raysThatHit.Count > 0)
            {
                ...
                if (!(hitUpper.collider) || (hitUpper.distance - raysThatHit[0].distance) > _minStepDepth)
                {
                    if (Vector3.Angle(raysThatHit[0].normal, _rigidbody.transform.TransformDirection(-_playerMoveInput)) <= _maximumAngleOfApproachToAscend)
                    {
                        ...
                        if (_isFirstStep)
                        {
                            calculatedStepInput = Quaternion.AngleAxis(45.0f, playerRelX) * calculatedStepInput * _ascendingStairsMovementMultiplier;
                            _isFirstStep = false;
                        }
                        else
                        {
                            ...
                            float Angle = Mathf.Atan2(stairHeight, avgDistance);
                            float tanAngle = 0.0f;
                            if (raysThatHit.Count < 10) // if the stairs are floating we have to increase the angle so that we can climb stairs while walking
                            {
                                tanAngle = (Angle + 0.5f) * Mathf.Rad2Deg;
                            }
                            else
                            {
                                tanAngle = Angle * Mathf.Rad2Deg;
                            }


                            // float tanAngle = (Mathf.Atan2(stairHeight, avgDistance) + 0.8f) * Mathf.Rad2Deg;
                    ...
                    else // more than 45* approch
                    {
                        _playerIsAscendingStairs = false;
                        _isFirstStep = true;
                    }
                }
                else // top ray hit something
                {
                    _playerIsAscendingStairs = false;
                    _isFirstStep = true;
                }
            ...
            if (raysThatHit.Count > 0)
            {
                ...
                if (!(hitUpper.collider) || (hitUpper.distance - raysThatHit[0].distance) > _minStepDepth)
                {
                    if (!(_playerIsGrounded) && hitUpper.distance < _capsuleCollider.radius + (_maxDescendRayDistance * 2.0f))
                    {
                        ...
                        Vector3 playerRelX = Vector3.Cross(_playerMoveInput, Vector3.up);
                        float stairHeight = 0.0f;
                        if (raysThatHit.Count < 10)
                        {
                            stairHeight = 20 * _rayIncrementAmount * _stairHeightPaddingMultiplier;
                        }
                        else
                        {
                            stairHeight = raysThatHit.Count * _rayIncrementAmount * _stairHeightPaddingMultiplier;
                        }
    ...
    private Vector3 PlayerSlope()
    {
        ...
        if (_playerIsGrounded && !(_playerIsAscendingStairs || _playerIsDescendingStairs))
        {
            ...
            if (groundSlopeAngle == 0.0f)
            {
                if (_input.MoveIsPressed)
                {
                    ...
                    if (Physics.Raycast(rayOrigin, _rigidbody.transform.TransformDirection(calculatedPlayerMovement), out rayHit, 0.75f))
                    {
                        ...
                        {
                            calculatedPlayerMovement.y = -120.0f;
                        }
                    }
            ...
            else
            {
                ...
                {
                    float calculatedSlopeGravity = groundSlopeAngle * -0.8f;
    ...
    private void Crouch()
    {
        if (_capsuleCollider.height >= _playerCrouchedHeight + _playerCrouchedHeightTolerance)
        {
            ...
            if (!(_jumpWasPressedLastFrame && !_playerIsGrounded)) // To move players rigidbody down only if its not jumping and is in the air;
            {
    ...
    private void UnCrouch()
    {
        ...
        {
            float sphereCastRadius = _capsuleCollider.radius * _headCheckRadiousMultiplier;
    ...
    private float PlayerFallGravity()
    {
        ...
        else
        {
            ...
            {
                float gravityFallMax = _movementMultiplier * _walkSpeedMultiplier * 0.3f;
